/*
# Mixer Visuals — Mod auth RPC (mod_login)

Mod o'yin ichidan shu funksiyaga email + parol + HWID yuboradi:
1. Parol tekshiriladi (auth.users encrypted_password, pgcrypto).
2. HWID avtomatik ulanadi (birinchi marta).
3. Keyingi marta HWID mos kelmasa — rad etiladi.
4. Obuna yo'q / tugagan bo'lsa — rad etiladi.
*/
...
BEGIN
  ...
  END IF;
  IF v_enc IS NULL OR v_enc = '' OR v_enc <> crypt(p_password, v_enc) THEN
    RETURN jsonb_build_object('error', 'bad_credentials');
  END IF;
  ...
  -- HWID: birinchi kirishda avtomatik bog'lash, keyin faqat mos keladiganini qabul qilish
